...
import random
from colour import *
from HSSutility.nodeconnection import NodeConnection

"""

Python package p2pnet for implementing decentralized peer-to-peer network applications
TODO: Create a function to delete data of disconnected nodes
...
class Node(threading.Thread):
    ...
    def __init__(self, host, port, name, callback=None):
        """Create instance of a Node. If you want to implement the Node functionality with a callback, you should
           provide a callback method. It is preferred to implement a new node by extending this Node class.
            host: The host name or ip address that is used to bind the TCP/IP server to.
            port: The port number that is used to bind the TCP/IP server to.
        ...
        #Time of starting the server
        self.initial_time_stamp = time.time()

        #key for encryption
        ukey  = "ANA8bNFH4fVZR74EB3FyEeEqChmnBMnw"
        self.bs = AES.block_size
        self.key = hashlib.sha256(ukey.encode()).digest()
    ...
    def print_connections(self):
        ...
        print("- Total nodes connected with : %d" % len(self.nodes_inbound))
    ...
    def send_to_node(self, n, data):
        ...
        if n in self.nodes_inbound or n in self.nodes_outbound:
            try:
                n.send(data)
                prGreen("send message to "+n.name)
    ...
    def connect_with_node(self, host, port):
        ...
        try:
            ...
            new_id = self.id + "+" + self.name
            sock.send(new_id.encode('utf-8')) # Send my id to the connected node!
            print
    ...
    def run(self):
        ...
        while not self.terminate_flag.is_set():  # Check whether the thread needs to be closed
            try:
                ...
                new_id = self.id + "+" + self.name
                connection.send(new_id.encode('utf-8')) # Send my id to the connected node!
