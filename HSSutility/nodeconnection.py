...
class NodeConnection(threading.Thread):
    ...
    def __str__(self):
        return 'NodeConnection: {}:{}::{} <-> {}:{}::{} ({})'.format(self.main_node.host, self.main_node.port, self.main_node.name, self.host, self.port,self.name, self.id)

    def __repr__(self):
        return '<NodeConnection: Node {}:{}::{} <-> Connection {}:{}::{}>'.format(self.main_node.host, self.main_node.port, self.main_node.name, self.host, self.port, self.name)
